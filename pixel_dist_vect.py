...
def px_dist(off_nadir, fov_across, fov_along, px_count, cam, rot_mat):
    ...
    px_at_nadir = get_area(np.array([off_nadir[0]])[:,np.newaxis], np.array([off_nadir[1]])[:,np.newaxis], [fov_across, fov_along], at_nadir_cam, at_nadir_rot_mat)[0][0][0]
...
off_nadir = [0*math.pi/180, 0*math.pi/180]
